 ...
 * @date: 2016-09-13
 */
public class dz_macro_pro_elec_contrast_activity extends BeamBaseActivity {
    ...
    private String curKpiName = "全省用电对比分析";
...
//        mChart.setMarkerView(marker);
    ...
    private void renderViewPager() {
        ...
        mViewPager.setAdapter(new FragmentStatePagerAdapter(getSupportFragmentManager()) {
            ...
            public Fragment getItem(int position) {
                ...
                dz_macro_pro_elec_contrast_table fragment = new dz_macro_pro_elec_contrast_table();
    ...
    private void renderData(int pos) {
        ...
        params.put("wheresql", RequestBody.create(MediaType.parse("text/plain"), " b.dscode = 'ELEREGION' "));
    ...
    private void renderData(String kpi) {
        ...
        Map<String, RequestBody> params = new HashMap<>();
        toolbarTitle.setText("山东省" + curNd + "年" + curYd + "月" + "月工业用电对比分析(万千瓦时、%)");

        params.put("type", RequestBody.create(MediaType.parse("text/plain"), "pro_contrast_elec_sql"));
    ...
    private BarData generateBarData(List<Map> rsList) {
        ...
        for (int index = 0; index < rsList.size() - 1; index++) {
            entries.add(new BarEntry(index + 1, Float.valueOf(String.valueOf(rsList.get(index + 1).get("valCm")))));
        }
    ...
    private LineData generateLineData(List<Map> rsList) {
        ...
        for (int index = 0; index < rsList.size() - 1; index++)
            entries.add(new Entry(index + 1f, Float.valueOf(String.valueOf(rsList.get(index + 1).get("valCmPy")))));
