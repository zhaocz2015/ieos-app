 ...
 * @date: 2016-09-13
 */
public class dz_macro_city_ib_activity extends BeamBaseActivity {
    ...
    private void renderViewPager() {
        ...
        mViewPager.setAdapter(new FragmentStatePagerAdapter(getSupportFragmentManager()) {
            ...
            public Fragment getItem(int position) {
                ...
                dz_macro_city_ib_table fragment = new dz_macro_city_ib_table();
    ...
    private void renderData(int pos) {
        ...
        params.put("wheresql", RequestBody.create(MediaType.parse("text/plain"), " b.dscode = 'IBSTATE' "));
    ...
    private void renderData(String kpi) {
        Map<String, RequestBody> params = new HashMap<>();
        toolbarTitle.setText(curNd + "年" + curYd + "月行业排名分析(万元、%)");

        params.put("type", RequestBody.create(MediaType.parse("text/plain"), "city_macro_ib_sql"));
        ...
        if(!AccountModel.getInstance().getUsername().equals("dzs")){
            params.put("whereMacSql", RequestBody.create(MediaType.parse("text/plain"), " and r.regionid = ( select s.regionid from ieos.sys_user s where s.username='" + AccountModel.getInstance().getUsername() + "' "));
        }
