 ...
 * @date: 2016-09-13
 */
public class dz_macro_city_elec_contrast_activity extends BeamBaseActivity {
...
//        mChart.setMarkerView(marker);
    ...
    private void renderViewPager() {
        ...
        mViewPager.setAdapter(new FragmentStatePagerAdapter(getSupportFragmentManager()) {
            ...
            public Fragment getItem(int position) {
                ...
                dz_macro_city_elec_contrast_table fragment = new dz_macro_city_elec_contrast_table();
    ...
    private void renderData(int pos) {
        ...
        params.put("wheresql", RequestBody.create(MediaType.parse("text/plain"), " b.dscode = 'ELECOUNTY' "));
    ...
    private void renderChartData() {
        // 重新绘制X轴
        mChart.getXAxis().setLabelCount(rsList.size() - 2);
        mChart.getXAxis().setAxisMaxValue(rsList.size() - 2);
        mChart.getXAxis().setValueFormatter(new AxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                int index = (int) value % (rsList.size() - 2);
                if (index == 0 || index == rsList.size() - 2) {
                    return "";
                }
    ...
    private BarData generateBarData(List<Map> rsList) {
        ...
        for (int index = 0; index < rsList.size() - 3; index++) {
